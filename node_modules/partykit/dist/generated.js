...
var source_default = {
  async fetch(request, env, ctx) {
    try {
      ...
      if (roomId) {
        ...
        if (request.headers.get("upgrade")?.toLowerCase() === "websocket" && Worker.onSocket) {
          const { 0: clientWebSocket, 1: serverWebSocket } = new WebSocketPair();
        ...
        if (url.pathname === "/__scheduled__" && typeof onCron === "function") {
          ...
          const cron = Object.entries(PARTYKIT_CRONS).find(
  ...
  async scheduled(controller, env, ctx) {
    ...
    if (typeof onCron === "function") {
      ...
      const cronName = Object.entries(PARTYKIT_CRONS).find(
        ([_name, val]) => val === cron
      )?.[0] || "unknown";
      return onCron(
